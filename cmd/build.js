...
if ( args.cloud_build_type ) {
    ...
    const versionPath = config.build.notCoverPublishMode ? `${version}/` : '';

    if ('daily' === args.cloud_build_type) {// 日常
        config[config.build_type].assetsPublicPath = `//assets-daily.test.youku.com/ku/child/${versionPath}`;
    } else {// 线上
        config[config.build_type].assetsPublicPath = `//g.alicdn.com/ku/child/${versionPath}`;
    }
    // 云构建类型
